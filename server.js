...
app.get('/setup-messenger', async (req, res) => {
  try {
    // 1. Bouton "Get Started" (premier message quand un utilisateur ouvre la conversation)
    await axios.post(
    ...
    // 2. Message de greeting (texte affiché avant que l'utilisateur n'écrive)
    await axios.post(
      `https://graph.facebook.com/v21.0/me/messenger_profile?access_token=${PAGE_ACCESS_TOKEN}`,
      {
        greeting: [
          {
            locale: 'default',
            text: 'Salama ! 👋 Bienvenue au Réseau PCM Ambolokandrina. Je peux t\'informer sur PCM, nos événements, notre bureau et bien plus. Envoie-moi un message !'
          },
          {
            locale: 'fr_FR',
            text: 'Salama ! 👋 Bienvenue au Réseau PCM Ambolokandrina. Je peux t\'informer sur PCM, nos événements, notre bureau et bien plus. Envoie-moi un message !'
          ...
          {
            locale: 'mg_MG',
            text: 'Salama ! 👋 Tongasoa amin\'ny RTP PCM Ambolokandrina. Afaka manome fampahalalana momba PCM, ny torohevitra, ny biraon\'ny mpitondra aho. Ramato ny hafatra !'
          }
        ]
      }
    );

    // 3. Menu persistent (bouton "Menu" en bas de l'écran)
    await axios.post(
      `https://graph.facebook.com/v21.0/me/messenger_profile?access_token=${PAGE_ACCESS_TOKEN}`,
      {
        persistent_menu: [
          {
            ...
            call_to_actions: [
              {
                type: 'postback',
                title: '📋 Qu\'est-ce que PCM ?',
                payload: 'QUICK_PCM_INFO'
              },
              {
                type: 'postback',
                title: '👥 Notre bureau',
                payload: 'QUICK_LEADERSHIP'
              },
              {
                type: 'postback',
                title: '📅 Événements',
                payload: 'QUICK_EVENTS'
              },
              {
                type: 'postback',
                title: '🎓 Bourses d\'études',
                payload: 'QUICK_SCHOLARSHIPS'
              },
              {
                type: 'postback',
                title: '🤝 Participer',
                payload: 'QUICK_PARTICIPATE'
              }
            ]
          }
        ]
    ...
    console.log('✅ Messenger profile configuré avec succès');
    res.json({ success: true, message: 'Messenger profile configuré (Get Started, Greeting, Menu)' });
  } catch (error) {
    console.error('Erreur setup-messenger:', error?.response?.data || error.message);
