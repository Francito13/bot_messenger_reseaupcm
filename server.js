...
app.get('/setup-messenger', async (req, res) => {
  ...
  // 2. Message de greeting (uniquement default + fr_FR, mg_MG peut poser problème)
  try {
    await axios.post(
      `https://graph.facebook.com/v21.0/me/messenger_profile?access_token=${PAGE_ACCESS_TOKEN}`,
      {
        greeting: [
          {
            locale: 'default',
            text: "Salama ! Bienvenue au Reseau PCM Ambolokandrina. Je peux t'informer sur PCM, nos evenements, notre bureau et bien plus. Envoie-moi un message !"
          }
        ]
      }
    );
    ...
    results.greeting = err?.response?.data?.error?.message || err.message;
  }
